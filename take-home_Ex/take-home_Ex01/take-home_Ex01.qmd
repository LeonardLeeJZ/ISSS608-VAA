...
```{r}
pacman::p_load(readr, ggrepel, patchwork, Hmisc, ggthemes, hrbrthemes, ggstatsplot, ggiraph, plotly, dplyr, patchwork, DT, tidyverse, ggplot2, stats)
```
...
## 3.3 Correcting Data Format [Jdata]
...
## 3.4.1 Correcting Data Format [Fdata]
...
### 5.1 Normality Testing of Joviality
...
# Calculate the quantile values for the Wage variable
quantiles <- quantile(Joined_data$Wage, probs = c(0.25, 0.5, 0.75))
...
income_levels <- cut(Joined_data$Wage, breaks, labels = c("Low", "Middle", "High"))

# Add the income levels as a new column in the dataset
Joined_data$Income_Level <- income_levels
```

### Comparison of mean Joviality Scores between Income Group Levels 
...
### Comparison of Means for Joviality across Education  Levels
...
Hi this is a placeholder text to see if this will stretch out the box

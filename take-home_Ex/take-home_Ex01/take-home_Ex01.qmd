...
```{r}
pacman::p_load(readr, ggrepel, patchwork, Hmisc, ggthemes, hrbrthemes, ggstatsplot, ggiraph, plotly, dplyr, patchwork, DT, tidyverse, ggplot2, stats, ggridges, viridis, scales, cowplot, ggdist, tidyquant, ggthemes, gt, rstatix, performance, see)
...
## Regression Model of Joviality

```{r}
# First plot
model1 <- lm(joviality ~ Education + Food + Recreation + 
...
test_kids <- ggbarstats(Joined_data,
                          x = haveKids, y = Income_Level, 
                        bf.message = FALSE)+
            labs(title = "Households with Lower Income have Lower Proportion of Children",
                 x = "Income Group") +
...
```{r}
set.seed(1234)
test_hhsize <- ggbarstats(Joined_data,
                          x = education_group, y = Income_Level,
                          bf.message = FALSE) +
            labs(title = "Large Proportion of High Income Earners are Highly Educated",
                 x = "Income Group") +
            theme(plot.background = element_rect(fill="#F8F3E6",colour="#F8F3E6"))
test_hhsize
...
### Income Level X Joviality x HaveKids

### Income Level X Joviality x Expense_Prop

...
Apart from the main dataset containing over 5.46 million links, there are a total of 12 Fish Bundles, for which we will visualise and compare to the MC2 data by matching them for source and target columns.
We will first import the datasets.
:::
...
::: {.cushbox .cushicon}
Apart from reorganising the nodes and edges using __select()__, we will also clean the dataset by removing duplicate rows, missing values in source/target columns, and renaming them.
